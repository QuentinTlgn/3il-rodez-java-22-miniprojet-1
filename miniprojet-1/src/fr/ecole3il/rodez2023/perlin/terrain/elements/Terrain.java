...
public class Terrain {
    ...
    public void setHydrometrie(double hydrometrie) {
        hydrometrie = Math.abs(hydrometrie); //Solution de secours (sensée être temporaire), car la valeur générée est parfois négative (??????)
    ...
    public TypeTerrain getTypeTerrain(DetermineurTerrain dt) {
        return dt.determinerTerrain(getAltitude(), getHydrometrie(), getTemperature());
    }
}
